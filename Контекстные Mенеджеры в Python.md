...
Контекстный менеджер в Python — это инструмент, который помогает безопасно работать с ресурсами и автоматически их освобождать.

Представь, что ты открываешь файл. Ты хочешь открыть файл, поработать с ним и обязательно закрыть его, даже если во время работы возникнет ошибка. Контекстный менеджер делает это за тебя:

- он задаёт начальные действия (например, открыть файл),
- даёт доступ к ресурсам внутри блока кода,
- и гарантирует, что ресурсы освободятся/закроются после выхода из блока, независимо от того, как завершится блок (нормально или с исключением).

Используется так:

```python
with открыть_файл('example.txt') as f:
    данные = f.read()
# файл автоматически закрывается здесь
```

Без контекстного менеджера тебе пришлось бы писать try/finally, чтобы явно закрыть файл. Контекстные менеджеры упрощают и делают код безопаснее.
-----------------------------------

Контекстный менеджер в Python — это объект, который управляет ресурсами и гарантирует их корректное освобождение после использования. Он применяется через оператор with и реализуется двумя основными способами: через класс с методами `__enter__` и `__exit__`, или через генератор с вспомогательной функцией `contextlib.contextmanager`.

1) Контекстный менеджер на основе класса
- Реализует методы `__enter__(self)` и `__exit__(self, exc_type, exc, tb)`.
- `__enter__` возвращает значение, которое будет доступно в теле блока `with`.
- `__exit__` вызывается при выходе из блока и принимает сведения об исключении (если оно было).
...
class FileOpener:
    ...
    def __enter__(self):
        ...
        return self.file  # доступен как переменная после "as"

    def __exit__(self, exc_type, exc, tb):
        if self.file:
            self.file.close()
        # Возвращает True, если исключение должно быть подавлено (обычно False)
        return False
...
2) Контекстные менеджеры через `contextlib`
- Упрощает создание контекстных менеджеров без явного определения класса.
...
- Основная идея: гарантировать вызов очистки независимо от того, произошло исключение или нет.
- Типичные примеры: работа с файлами, сетевыми соединениями, локами, транзакциями баз данных и т.д.
