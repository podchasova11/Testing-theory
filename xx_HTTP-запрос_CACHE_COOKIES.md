### Как в GET запросе отправить через квери параметр два параметра. Синтаксически как это сделать? (Вопрос из Собеса  РТК_ИТ)
...
### Eсли программист говорит это не баг, это фича тестировщику

Смысл в том, что иногда то, что с виду кажется ошибкой, специально сделано как особенность (фича). 

Как правильно это использовать и распознавать:
- Отличие от баг-фикса: фича должна быть документирована (справка по API, changelog, issue description). Без документации это риск — может привести к недопониманию.
- Взаимоотношения с тестировщиком: хорошая практика — заранее обсуждать такие решения и добавлять тест-кейсы, которые подтверждают, что это поведение не считается багом.
- Этическая сторона: если реальное ожидание пользователя не совпадает с поведением, фича может скрывать проблемы UX. В таких случаях стоит прояснить цель и собрать критерии приемки.

Примеры формулировок:
- Вежливо и конструктивно: "Это не баг, это запланированная фича. Давай добавим тест и описание в документацию."
- Обоснование решения: "Поведение реализовано как фича согласно спецификации, которая требует такого формата вывода при определенных условиях."
- Если спор: "Понимаю, что выглядит как баг. Можем обсудить критерии приемки и обновить документацию, чтобы избежать недопонимания в будущем."
### Oсновные форматы типов JSON

Вот основные форматы и схемы представления JSON-данных, которые часто встречаются в API и конфигурациях:

1. Объект (object) a в Python, это библиотека - dict 

- Содержит набор пар ключ-значение.
- Ключи – строки.
- Значения могут быть любыми допустимыми типами JSON.
...
- Значения могут быть любыми допустимыми типами JSON.
- Синтаксис: [ значение1, значение2, значение3, ... ]
...
3. Примитивы (primitive types)
- string: "text" (двойные кавычки)
- number: целые или с плавающей точкой, без кавычек
  - Примеры: 42, -3.14, 0.001
- boolean: true или false
- null: null


4. Специфичные ограничения и хитрости
- Ключи в объектах должны быть строками.
...
### Что есть в GET запросе и чего нету в POST

Кратко: в GET-запросе данные передаются в URL (через строку запроса), а в POST-запросе данные обычно передаются в теле запроса. Это влияет на видимость данных, размер и использование запросов.

Подробно:

- Что есть в GET запросе
  - Данные в URL: параметры добавляются после знака ?, например: /search?q=россия&sort=price
  - Видимость: данные видны в адресной строке и в журналах сервера/браузера
  - Ограничение по размеру: обычно ограничение длины URL (размер зависит от браузера и сервера; часто ~2000–8000 символов)
  - Идемпотентность: GET должен быть идемпотентным и безопасным — повторные запросы не должны изменять состояние сервера
  - Кэшируемость: ответы на GET часто кэшируются

- Что есть в POST запросе
  - Данные в теле запроса: параметры не видны в URL; чаще используется application/x-www-form-urlencoded, multipart/form-data, либо JSON (application/json)
  - Видимость: данные не отображаются в адресной строке; лучше скрыты в пределах тела запроса
  - Размер: тело запроса может быть значительно больше, ограничение размером тела со стороны сервера
  - Не идемпотентность по умолчанию: POST обычно используется для создания или изменения состояния на сервере; повторный POST может привести к повторным операциям
  - Кэширование: ответы на POST обычно не кэшируются без специальных заголовков

- Что часто отличается по практике
  - Безопасность: чувствительные данные НЕ рекомендуется отправлять через GET (логирование URL, кэширование); для чувствительных данных предпочтительнее POST с соответствующей настройкой безопасности (HTTPS, отсутствие логирования)
  - Idempotence: GET — идемпотентен; POST — не обязательно
  - Использование: GET — для получения данных, поиск, фильтрация; POST — для создания, обновления, действий, которые изменяют серверное состояние

Если нужно, могу привести примеры: 
- GET пример: GET /api/products?category=books&limit=10
- POST пример: POST /api/products с телом { "name": "Книга", "price": 9.99 }

Скажите, в каком контексте нужна детализация: для обучения, документации API, или для объяснения резюме/презентации?
