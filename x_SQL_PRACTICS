C:\>cd "Program files"
C:\Program Files>cd MySQL
C:\Program Files\MySQL>cd "MySQL Server 8.0"
C:\Program Files\MySQL\MySQL Server 8.0>cd bin
C:\Program Files\MySQL\MySQL Server 8.0\bin>mysql -u root -p
Enter password: ****


# Чтобы найти все департаменты, в которых нет ни одного сотрудника:
SELECT d.name
FROM departamens d
LEFT JOIN users u ON d.id = u.departament_id
WHERE u.departament_id IS NULL;

# вывести всех клиентов, у которых сумма платежей больше 2000р
  в выводе должно быть: client_id, и средняя сумма платежа. 
SELECT t.client_id, tclient_id, avg(tp.amount)
FROM t_account t JOIN t_payments tp ON t.id = tp.account_id
GROUP BY t.client_id
HAVING avg(tp.amount) > 2000;

# получить список всех заказов вместе с именами клиентов 
из таблиц Customer c, Orders o

SELECT O.order_id, C.costumer_name 
FROM Orders o INNER JOIN Custumers c ON o.costumer_id = c.costumer.id;


    Напишите решение для поиска всех авторов, которые просмотрели 
    хотя бы одну из своих статей. Верните таблицу результатов, 
    отсортированную по идентификатору в порядке возрастания.

SELECT DISTINCT author_id AS id
FROM Views
WHERE author_id = viewer_id
ORDER BY id;

SELECT name, population, area
FROM world
WHERE area >= 300000 
OR population >= 25000000;


SHOW DATABASES;
CREATE DATABASE sql_course;
SHOW DATABASES;
DROP DATABASE sql_course;
SHOW DATABASES;
CREATE DATABASE sql_course;
SHOW DATABASES;
USE sql_course;
show tables;

 CREATE TABLE teacher(
    -> id INT AUTO_INCREMENT PRIMARY KEY,
    -> surname VARCHAR(255) NOT NULL
    -> );


show tables;
show columns FROM teacher;

CREATE TABLE lesson(
    -> id INT AUTO_INCREMENT PRIMARY KEY,
    -> name VARCHAR(255) NOT NULL,
    -> teacher_id INT NOT NULL,
    -> FOREIGN KEY (teacher_id) references teacher(id)
    -> );
	
show tables;
show columns from lesson;

INSERT INTO teacher (surname) values ("Иванов");
INSERT INTO teacher (surname) values ("Иванов");
INSERT INTO teacher (surname) values ("Петров");
INSERT INTO teacher (surname) values ("Сидоров");
INSERT INTO teacher (surname) values ("Чижов");	
INSERT INTO teacher (surname) values ("Петров");
INSERT INTO teacher (surname) values ("Петров");

SELECT * FROM teacher;

SELECT id FROM teacher;
SELECT id, surname FROM teacher;
SELECT DISTINCT surname FROM teacher;
SELECT * FROM teacher WHERE id = 1;
SELECT * FROM teacher WHERE id = 3;
SELECT * FROM teacher WHERE id > 3;

SELECT * FROM teacher WHERE surname = "Петров";
SELECT * FROM teacher WHERE surname = "Петров" LIMIT 2;
SELECT * FROM teacher WHERE surname = "Петров" LIMIT 1;
SELECT * FROM teacher LIMIT 5;

SELECT id AS 'ИДЕНТИФИКАТОР', surname AS 'Фамилия' FROM teacher;

SELECT * FROM teacher ORDER BY surname;
SELECT * FROM teacher ORDER BY id
SELECT * FROM teacher ORDER BY id DESC;

ALTER TABLE teacher ADD age INT;
show columns from teacher;
select * from teacher;

UPDATE teacher SET age = 20 where id=1;
select * from teacher;
UPDATE teacher SET age = 25;

UPDATE teacher SET age = 30 where id=2;
UPDATE teacher SET age = 35 where id=3;
UPDATE teacher SET age = 40 where id=4;
UPDATE teacher SET age = 45 where id=5;
UPDATE teacher SET age = 50 where id=6;

select * from teacher;
SELECT * FROM teacher WHERE surname LIKE "%ов";
SELECT * FROM teacher WHERE surname LIKE "п%ов";
SELECT * FROM teacher WHERE id > 3 AND age < 45;
SELECT * FROM teacher WHERE id > 3 AND age < 46;
SELECT * FROM teacher WHERE id > 4 AND age > 31;
SELECT * FROM teacher WHERE NOT id = 2;
SELECT * FROM teacher WHERE age BETWEEN 35 and 45;

DELETE FROM teacher WHERE id = 6;
SELECT * FROM teacher;

INSERT INTO lesson (name, teacher_id) VALUES ("Математика", 1), ("Информатика", 1), ("Русский", 2), ("Физика", 3);

select * from lesson;

SELECT teacher.surname, lesson.name FROM teacher INNER JOIN lesson ON teacher.id = lesson.teacher_id;
SELECT teacher.surname, lesson.name FROM teacher LEFT OUTER JOIN lesson ON teacher.id = lesson.teacher_id;
SELECT teacher.surname, lesson.name FROM teacher RIGHT OUTER JOIN lesson ON teacher.id = lesson.teacher_id;

ВЕРТИКАЛЬНОЕ ОБЪЕДИНЕНИЕ ТАБЛИЦ
SELECT * FROM teacher UNION SELECT * FROM lesson;
SELECT * FROM lesson UNION SELECT * FROM teacher;

ФУНКЦИИ: AVG => СРЕДНЕЕ ЗНАЧЕНИЕ, MAX, MIN, SUM ...
SELECT AVG(age) FROM teacher;
+----------+
| AVG(age) |
+----------+
|  35.0000 |
+----------+

SELECT MAX(age), MIN(age) FROM teacher;
+----------+----------+
| MAX(age) | MIN(age) |
+----------+----------+
|       45 |       25 |
+----------+----------+

SELECT SUM(age) FROM teacher;
+----------+
| SUM(age) |
+----------+
|      175 |
+----------+

INSERT INTO teacher (surname, age) VALUES ("Николаев", 25), ("Шишкин", 30);

select * from teacher;
+----+----------+------+
| id | surname  | age  |
+----+----------+------+
|  1 | Иванов   |   25 |
|  2 | Петров   |   30 |
|  3 | Сидоров  |   35 |
|  4 | Чижов    |   40 |
|  5 | Петров   |   45 |
|  7 | Николаев |   25 |
|  8 | Шишкин   |   30 |
+----+----------+------+

SELECT age, COUNT(age) FROM teacher GROUP BY age;
+------+------------+
| age  | COUNT(age) |
+------+------------+
|   25 |          2 |
|   30 |          2 |
|   35 |          1 |
|   40 |          1 |
|   45 |          1 |
+------+------------+

INSERT INTO teacher (surname, age) VALUES ("Николаев", 25), ("Шишкин", 30);

select * from teacher;
+----+----------+------+
| id | surname  | age  |
+----+----------+------+
|  1 | Иванов   |   25 |
|  2 | Петров   |   30 |
|  3 | Сидоров  |   35 |
|  4 | Чижов    |   40 |
|  5 | Петров   |   45 |
|  7 | Николаев |   25 |
|  8 | Шишкин   |   30 |
+----+----------+------+

SELECT age, COUNT(age) FROM teacher GROUP BY age;
+------+------------+
| age  | COUNT(age) |
+------+------------+
|   25 |          2 |
|   30 |          2 |
|   35 |          1 |
|   40 |          1 |
|   45 |          1 |
+------+------------+



mysql> SHOW DATABASES;
+--------------------+
| Database           |
+--------------------+
| information_schema |
| mysql              |
| performance_schema |
| sakila             |
| sys                |
| test_1             |
| world              |
+--------------------+
7 rows in set (0.00 sec)

mysql> CREATE DATABASE sql_course;
Query OK, 1 row affected (0.01 sec)

mysql> SHOW DATABASES;
+--------------------+
| Database           |
+--------------------+
| information_schema |
| mysql              |
| performance_schema |
| sakila             |
| sql_course         |
| sys                |
| test_1             |
| world              |
+--------------------+
8 rows in set (0.00 sec)

mysql> DROP DATABASE sql_course;
Query OK, 0 rows affected (0.01 sec)

mysql> SHOW DATABASES;
+--------------------+
| Database           |
+--------------------+
| information_schema |
| mysql              |
| performance_schema |
| sakila             |
| sys                |
| test_1             |
| world              |
+--------------------+
7 rows in set (0.00 sec)

mysql> CREATE DATABASE sql_course;
Query OK, 1 row affected (0.00 sec)

mysql> SHOW DATABASES;
+--------------------+
| Database           |
+--------------------+
| information_schema |
| mysql              |
| performance_schema |
| sakila             |
| sql_course         |
| sys                |
| test_1             |
| world              |
+--------------------+
8 rows in set (0.00 sec)

mysql> USE sql_course;
Database changed

mysql> show tables;
Empty set (0.00 sec)



mysql> CREATE TABLE teacher(
    -> id INT AUTO_INCREMENT PRIMARY KEY,
    -> surname VARCHAR(255) NOT NULL
    -> );
	
	mysql> show tables;
+----------------------+
| Tables_in_sql_course |
+----------------------+
| teacher              |
+----------------------+
1 row in set (0.00 sec)

mysql> show columns FROM teacher;
+---------+--------------+------+-----+---------+----------------+
| Field   | Type         | Null | Key | Default | Extra          |
+---------+--------------+------+-----+---------+----------------+
| id      | int          | NO   | PRI | NULL    | auto_increment |
| surname | varchar(255) | NO   |     | NULL    |                |
+---------+--------------+------+-----+---------+----------------+
2 rows in set (0.00 sec)

mysql> CREATE TABLE lesson(
    -> id INT AUTO_INCREMENT PRIMARY KEY,
    -> name VARCHAR(255) NOT NULL,
    -> teacher_id INT NOT NULL,
    -> FOREIGN KEY (teacher_id) references teacher(id)
    -> );
Query OK, 0 rows affected (0.02 sec)

mysql> show tables;
+----------------------+
| Tables_in_sql_course |
+----------------------+
| lesson               |
| teacher              |
+----------------------+
2 rows in set (0.00 sec)

mysql> show columns from lesson;
+------------+--------------+------+-----+---------+----------------+
| Field      | Type         | Null | Key | Default | Extra          |
+------------+--------------+------+-----+---------+----------------+
| id         | int          | NO   | PRI | NULL    | auto_increment |
| name       | varchar(255) | NO   |     | NULL    |                |
| teacher_id | int          | NO   | MUL | NULL    |                |
+------------+--------------+------+-----+---------+----------------+
3 rows in set (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Иванов");
Query OK, 1 row affected (0.01 sec)

mysql> INSERT INTO teacher (surname) values ("Иванов");
Query OK, 1 row affected (0.01 sec)

mysql> INSERT INTO teacher (surname) values ("Петров");
Query OK, 1 row affected (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Сидоров");
Query OK, 1 row affected (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Чижов");
Query OK, 1 row affected (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Петров");
Query OK, 1 row affected (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Петров");
Query OK, 1 row affected (0.00 sec)

mysql> SELECT * FROM teacher;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
|  3 | Сидоров |
|  4 | Чижов   |
|  5 | Петров  |
|  6 | Петров  |
+----+---------+
6 rows in set (0.00 sec)

mysql> SELECT id FROM teacher;
+----+
| id |
+----+
|  1 |
|  2 |
|  3 |
|  4 |
|  5 |
|  6 |
+----+
6 rows in set (0.00 sec)

mysql> SELECT id, surname, surname FROM teacher;
+----+---------+---------+
| id | surname | surname |
+----+---------+---------+
|  1 | Иванов  | Иванов  |
|  2 | Петров  | Петров  |
|  3 | Сидоров | Сидоров |
|  4 | Чижов   | Чижов   |
|  5 | Петров  | Петров  |
|  6 | Петров  | Петров  |
+----+---------+---------+
6 rows in set (0.00 sec)

mysql> SELECT DISTINCT surname FROM teacher;
+---------+
| surname |
+---------+
| Иванов  |
| Петров  |
| Сидоров |
| Чижов   |
+---------+
4 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id = 1;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
+----+---------+
1 row in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id = 3;
+----+---------+
| id | surname |
+----+---------+
|  3 | Сидоров |
+----+---------+
1 row in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id > 3;
+----+---------+
| id | surname |
+----+---------+
|  4 | Чижов   |
|  5 | Петров  |
|  6 | Петров  |
+----+---------+
3 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id < 3;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
+----+---------+
2 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE surname = "Петров";
+----+---------+
| id | surname |
+----+---------+
|  2 | Петров  |
|  5 | Петров  |
|  6 | Петров  |
+----+---------+
3 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE surname = "Петров" LIMIT 2;
+----+---------+
| id | surname |
+----+---------+
|  2 | Петров  |
|  5 | Петров  |
+----+---------+
2 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE surname = "Петров" LIMIT 1;
+----+---------+
| id | surname |
+----+---------+
|  2 | Петров  |
+----+---------+
1 row in set (0.00 sec)

mysql> SELECT * FROM teacher LIMIT 5;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
|  3 | Сидоров |
|  4 | Чижов   |
|  5 | Петров  |
+----+---------+
5 rows in set (0.00 sec)

mysql> SELECT id AS 'ИДЕНТИФИКАТОР', surname AS 'Фамилия' FROM teacher;
+---------------+---------+
| ИДЕНТИФИКАТОР | Фамилия |
+---------------+---------+
|             1 | Иванов  |
|             2 | Петров  |
|             3 | Сидоров |
|             4 | Чижов   |
|             5 | Петров  |
|             6 | Петров  |
+---------------+---------+
6 rows in set (0.00 sec)

mysql> SELECT * FROM teacher ORDER BY surname;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
|  5 | Петров  |
|  6 | Петров  |
|  3 | Сидоров |
|  4 | Чижов   |
+----+---------+
6 rows in set (0.00 sec)

mysql> SELECT * FROM teacher ORDER BY id;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
|  3 | Сидоров |
|  4 | Чижов   |
|  5 | Петров  |
|  6 | Петров  |
+----+---------+
6 rows in set (0.00 sec)

mysql> SELECT * FROM teacher ORDER BY id DESC;
+----+---------+
| id | surname |
+----+---------+
|  6 | Петров  |
|  5 | Петров  |
|  4 | Чижов   |
|  3 | Сидоров |
|  2 | Петров  |
|  1 | Иванов  |
+----+---------+
6 rows in set (0.00 sec)

mysql> ALTER TABLE teacher ADD age INT;
Query OK, 0 rows affected (0.02 sec)
Records: 0  Duplicates: 0  Warnings: 0

mysql> show columns from teacher;
+---------+--------------+------+-----+---------+----------------+
| Field   | Type         | Null | Key | Default | Extra          |
+---------+--------------+------+-----+---------+----------------+
| id      | int          | NO   | PRI | NULL    | auto_increment |
| surname | varchar(255) | NO   |     | NULL    |                |
| age     | int          | YES  |     | NULL    |                |
+---------+--------------+------+-----+---------+----------------+
3 rows in set (0.00 sec)

mysql> select * from teacher;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  | NULL |
|  2 | Петров  | NULL |
|  3 | Сидоров | NULL |
|  4 | Чижов   | NULL |
|  5 | Петров  | NULL |
|  6 | Петров  | NULL |
+----+---------+------+
6 rows in set (0.00 sec)

mysql> UPDATE teacher SET age = 20 where id=1;
Query OK, 1 row affected (0.00 sec)
Rows matched: 1  Changed: 1  Warnings: 0

mysql> select * from teacher;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  |   20 |
|  2 | Петров  | NULL |
|  3 | Сидоров | NULL |
|  4 | Чижов   | NULL |
|  5 | Петров  | NULL |
|  6 | Петров  | NULL |
+----+---------+------+
6 rows in set (0.00 sec)

mysql> UPDATE teacher SET age = 25;
Query OK, 6 rows affected (0.00 sec)
Rows matched: 6  Changed: 6  Warnings: 0

mysql> select * from teacher;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  |   25 |
|  2 | Петров  |   25 |
|  3 | Сидоров |   25 |
|  4 | Чижов   |   25 |
|  5 | Петров  |   25 |
|  6 | Петров  |   25 |
+----+---------+------+
6 rows in set (0.00 sec)

mysql> UPDATE teacher SET age = 30 where id=2;
Query OK, 1 row affected (0.00 sec)
Rows matched: 1  Changed: 1  Warnings: 0

mysql> UPDATE teacher SET age = 35 where id=3;
Query OK, 1 row affected (0.00 sec)
Rows matched: 1  Changed: 1  Warnings: 0

mysql> UPDATE teacher SET age = 40 where id=4;
Query OK, 1 row affected (0.00 sec)
Rows matched: 1  Changed: 1  Warnings: 0

mysql> UPDATE teacher SET age = 45 where id=5;
Query OK, 1 row affected (0.00 sec)
Rows matched: 1  Changed: 1  Warnings: 0

mysql> UPDATE teacher SET age = 50 where id=6;
Query OK, 1 row affected (0.00 sec)
Rows matched: 1  Changed: 1  Warnings: 0

mysql> select * from teacher;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  |   25 |
|  2 | Петров  |   30 |
|  3 | Сидоров |   35 |
|  4 | Чижов   |   40 |
|  5 | Петров  |   45 |
|  6 | Петров  |   50 |
+----+---------+------+
6 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE surname LIKE "%ов";
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  |   25 |
|  2 | Петров  |   30 |
|  3 | Сидоров |   35 |
|  4 | Чижов   |   40 |
|  5 | Петров  |   45 |
|  6 | Петров  |   50 |
+----+---------+------+
6 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE surname LIKE "п%ов";
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  2 | Петров  |   30 |
|  5 | Петров  |   45 |
|  6 | Петров  |   50 |
+----+---------+------+
3 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id > 3 AND age < 45;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  4 | Чижов   |   40 |
+----+---------+------+
1 row in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id > 3 AND age < 46;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  4 | Чижов   |   40 |
|  5 | Петров  |   45 |
+----+---------+------+
2 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id > 4 AND age > 31;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  5 | Петров  |   45 |
|  6 | Петров  |   50 |
+----+---------+------+
2 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE NOT id = 2;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  |   25 |
|  3 | Сидоров |   35 |
|  4 | Чижов   |   40 |
|  5 | Петров  |   45 |
|  6 | Петров  |   50 |
+----+---------+------+
5 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE age BETWEEN 35 and 45;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  3 | Сидоров |   35 |
|  4 | Чижов   |   40 |
|  5 | Петров  |   45 |
+----+---------+------+
3 rows in set (0.00 sec)

mysql> DELETE FROM teacher WHERE id = 6;
Query OK, 1 row affected (0.00 sec)

mysql> SELECT * FROM teacher;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  |   25 |
|  2 | Петров  |   30 |
|  3 | Сидоров |   35 |
|  4 | Чижов   |   40 |
|  5 | Петров  |   45 |
+----+---------+------+
5 rows in set (0.00 sec)

mysql> INSERT INTO lesson (name, teacher_id) VALUES ("Математика", 1), ("Информатика", 1), ("Русский", 2), ("Физика", 3);
Query OK, 4 rows affected (0.01 sec)
Records: 4  Duplicates: 0  Warnings: 0

mysql> select * from lesson;
+----+-------------+------------+
| id | name        | teacher_id |
+----+-------------+------------+
|  1 | Математика  |          1 |
|  2 | Информатика |          1 |
|  3 | Русский     |          2 |
|  4 | Физика      |          3 |
|  5 | Математика  |          1 |
|  6 | Информатика |          1 |
|  7 | Русский     |          2 |
|  8 | Физика      |          3 |
+----+-------------+------------+
8 rows in set (0.00 sec)

mysql> SELECT teacher.surname, lesson.name FROM teacher INNER JOIN lesson ON teacher.id = lesson.teacher_id;
+---------+-------------+
| surname | name        |
+---------+-------------+
| Иванов  | Математика  |
| Иванов  | Информатика |
| Иванов  | Математика  |
| Иванов  | Информатика |
| Петров  | Русский     |
| Петров  | Русский     |
| Сидоров | Физика      |
| Сидоров | Физика      |
+---------+-------------+
8 rows in set (0.00 sec)

mysql> SELECT teacher.surname, lesson.name FROM teacher LEFT OUTER JOIN lesson ON teacher.id = lesson.teacher_id;
+---------+-------------+
| surname | name        |
+---------+-------------+
| Иванов  | Математика  |
| Иванов  | Информатика |
| Иванов  | Математика  |
| Иванов  | Информатика |
| Петров  | Русский     |
| Петров  | Русский     |
| Сидоров | Физика      |
| Сидоров | Физика      |
| Чижов   | NULL        |
| Петров  | NULL        |
+---------+-------------+
10 rows in set (0.00 sec)

mysql> SELECT teacher.surname, lesson.name FROM teacher RIGHT OUTER JOIN lesson ON teacher.id = lesson.teacher_id;
+---------+-------------+
| surname | name        |
+---------+-------------+
| Иванов  | Математика  |
| Иванов  | Информатика |
| Петров  | Русский     |
| Сидоров | Физика      |
| Иванов  | Математика  |
| Иванов  | Информатика |
| Петров  | Русский     |
| Сидоров | Физика      |
+---------+-------------+
8 rows in set (0.00 sec)

mysql> ВЕРТИКАЛЬНОЕ ОБЪЕДИНЕНИЕ ТАБЛИЦ

mysql> SELECT * FROM teacher UNION SELECT * FROM lesson;
+----+-------------+------+
| id | surname     | age  |
+----+-------------+------+
|  1 | Иванов      |   25 |
|  2 | Петров      |   30 |
|  3 | Сидоров     |   35 |
|  4 | Чижов       |   40 |
|  5 | Петров      |   45 |
|  1 | Математика  |    1 |
|  2 | Информатика |    1 |
|  3 | Русский     |    2 |
|  4 | Физика      |    3 |
|  5 | Математика  |    1 |
|  6 | Информатика |    1 |
|  7 | Русский     |    2 |
|  8 | Физика      |    3 |
+----+-------------+------+
13 rows in set (0.00 sec)

mysql> SELECT * FROM lesson UNION SELECT * FROM teacher;
+----+-------------+------------+
| id | name        | teacher_id |
+----+-------------+------------+
|  1 | Математика  |          1 |
|  2 | Информатика |          1 |
|  3 | Русский     |          2 |
|  4 | Физика      |          3 |
|  5 | Математика  |          1 |
|  6 | Информатика |          1 |
|  7 | Русский     |          2 |
|  8 | Физика      |          3 |
|  1 | Иванов      |         25 |
|  2 | Петров      |         30 |
|  3 | Сидоров     |         35 |
|  4 | Чижов       |         40 |
|  5 | Петров      |         45 |
+----+-------------+------------+
13 rows in set (0.00 sec)


ФУНКЦИИ: AVG => СРЕДНЕЕ ЗНАЧЕНИЕ, MAX, MIN, SUM ...

mysql> SELECT AVG(age) FROM teacher;
+----------+
| AVG(age) |
+----------+
|  35.0000 |
+----------+
1 row in set (0.00 sec)

mysql> SELECT MAX(age), MIN(age) FROM teacher;
+----------+----------+
| MAX(age) | MIN(age) |
+----------+----------+
|       45 |       25 |
+----------+----------+
1 row in set (0.00 sec)

mysql> SELECT SUM(age) FROM teacher;
+----------+
| SUM(age) |
+----------+
|      175 |
+----------+
1 row in set (0.00 sec)

mysql> INSERT INTO teacher (surname, age) VALUES ("Николаев", 25), ("Шишкин", 30);
Query OK, 2 rows affected (0.00 sec)
Records: 2  Duplicates: 0  Warnings: 0

mysql> select * from teacher;
+----+----------+------+
| id | surname  | age  |
+----+----------+------+
|  1 | Иванов   |   25 |
|  2 | Петров   |   30 |
|  3 | Сидоров  |   35 |
|  4 | Чижов    |   40 |
|  5 | Петров   |   45 |
|  7 | Николаев |   25 |
|  8 | Шишкин   |   30 |
+----+----------+------+

mysql> SELECT age, COUNT(age) FROM teacher;
ERROR 1140 (42000): In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'sql_course.teacher.age'; this is incompatible with sql_mode=only_full_group_by


mysql> SELECT age, COUNT(age) FROM teacher GROUP BY age;
+------+------------+
| age  | COUNT(age) |
+------+------------+
|   25 |          2 |
|   30 |          2 |
|   35 |          1 |
|   40 |          1 |
|   45 |          1 |
+------+------------+
5 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id = 1;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  |   25 |
+----+---------+------+
1 row in set (0.00 sec)

mysql>

mysql> SELECT * FROM teacher WHERE id > 1;
+----+----------+------+
| id | surname  | age  |
+----+----------+------+
|  2 | Петров   |   30 |
|  3 | Сидоров  |   35 |
|  4 | Чижов    |   40 |
|  5 | Петров   |   45 |
|  7 | Николаев |   25 |
|  8 | Шишкин   |   30 |
+----+----------+------+
6 rows in set (0.00 sec)

INSERT INTO teacher (surname, age) VALUES ("Николаев", 25), ("Шишкин", 30);

select * from teacher;
+----+----------+------+
| id | surname  | age  |
+----+----------+------+
|  1 | Иванов   |   25 |
|  2 | Петров   |   30 |
|  3 | Сидоров  |   35 |
|  4 | Чижов    |   40 |
|  5 | Петров   |   45 |
|  7 | Николаев |   25 |
|  8 | Шишкин   |   30 |
+----+----------+------+

SELECT age, COUNT(age) FROM teacher GROUP BY age;
+------+------------+
| age  | COUNT(age) |
+------+------------+
|   25 |          2 |
|   30 |          2 |
|   35 |          1 |
|   40 |          1 |
|   45 |          1 |
+------+------------+



mysql> SHOW DATABASES;
+--------------------+
| Database           |
+--------------------+
| information_schema |
| mysql              |
| performance_schema |
| sakila             |
| sys                |
| test_1             |
| world              |
+--------------------+
7 rows in set (0.00 sec)

mysql> CREATE DATABASE sql_course;
Query OK, 1 row affected (0.01 sec)

mysql> SHOW DATABASES;
+--------------------+
| Database           |
+--------------------+
| information_schema |
| mysql              |
| performance_schema |
| sakila             |
| sql_course         |
| sys                |
| test_1             |
| world              |
+--------------------+
8 rows in set (0.00 sec)

mysql> DROP DATABASE sql_course;
Query OK, 0 rows affected (0.01 sec)

mysql> SHOW DATABASES;
+--------------------+
| Database           |
+--------------------+
| information_schema |
| mysql              |
| performance_schema |
| sakila             |
| sys                |
| test_1             |
| world              |
+--------------------+
7 rows in set (0.00 sec)

mysql> CREATE DATABASE sql_course;
Query OK, 1 row affected (0.00 sec)

mysql> SHOW DATABASES;
+--------------------+
| Database           |
+--------------------+
| information_schema |
| mysql              |
| performance_schema |
| sakila             |
| sql_course         |
| sys                |
| test_1             |
| world              |
+--------------------+
8 rows in set (0.00 sec)

mysql> USE sql_course;
Database changed

mysql> show tables;
Empty set (0.00 sec)


mysql> CREATE TABLE teacher(
    -> id INT AUTO_INCREMENT PRIMARY KEY,
    -> surname VARCHAR(255) NOT NULL
    -> );
	
	mysql> show tables;
+----------------------+
| Tables_in_sql_course |
+----------------------+
| teacher              |
+----------------------+
1 row in set (0.00 sec)

mysql> show columns FROM teacher;
+---------+--------------+------+-----+---------+----------------+
| Field   | Type         | Null | Key | Default | Extra          |
+---------+--------------+------+-----+---------+----------------+
| id      | int          | NO   | PRI | NULL    | auto_increment |
| surname | varchar(255) | NO   |     | NULL    |                |
+---------+--------------+------+-----+---------+----------------+
2 rows in set (0.00 sec)

mysql> CREATE TABLE lesson(
    -> id INT AUTO_INCREMENT PRIMARY KEY,
    -> name VARCHAR(255) NOT NULL,
    -> teacher_id INT NOT NULL,
    -> FOREIGN KEY (teacher_id) references teacher(id)
    -> );
Query OK, 0 rows affected (0.02 sec)

mysql> show tables;
+----------------------+
| Tables_in_sql_course |
+----------------------+
| lesson               |
| teacher              |
+----------------------+
2 rows in set (0.00 sec)

mysql> show columns from lesson;
+------------+--------------+------+-----+---------+----------------+
| Field      | Type         | Null | Key | Default | Extra          |
+------------+--------------+------+-----+---------+----------------+
| id         | int          | NO   | PRI | NULL    | auto_increment |
| name       | varchar(255) | NO   |     | NULL    |                |
| teacher_id | int          | NO   | MUL | NULL    |                |
+------------+--------------+------+-----+---------+----------------+
3 rows in set (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Иванов");
Query OK, 1 row affected (0.01 sec)

mysql> INSERT INTO teacher (surname) values ("Иванов");
Query OK, 1 row affected (0.01 sec)

mysql> INSERT INTO teacher (surname) values ("Петров");
Query OK, 1 row affected (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Сидоров");
Query OK, 1 row affected (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Чижов");
Query OK, 1 row affected (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Петров");
Query OK, 1 row affected (0.00 sec)

mysql> INSERT INTO teacher (surname) values ("Петров");
Query OK, 1 row affected (0.00 sec)

mysql> SELECT * FROM teacher;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
|  3 | Сидоров |
|  4 | Чижов   |
|  5 | Петров  |
|  6 | Петров  |
+----+---------+
6 rows in set (0.00 sec)

mysql> SELECT id FROM teacher;
+----+
| id |
+----+
|  1 |
|  2 |
|  3 |
|  4 |
|  5 |
|  6 |
+----+
6 rows in set (0.00 sec)

mysql> SELECT id, surname, surname FROM teacher;
+----+---------+---------+
| id | surname | surname |
+----+---------+---------+
|  1 | Иванов  | Иванов  |
|  2 | Петров  | Петров  |
|  3 | Сидоров | Сидоров |
|  4 | Чижов   | Чижов   |
|  5 | Петров  | Петров  |
|  6 | Петров  | Петров  |
+----+---------+---------+
6 rows in set (0.00 sec)

mysql> SELECT DISTINCT surname FROM teacher;
+---------+
| surname |
+---------+
| Иванов  |
| Петров  |
| Сидоров |
| Чижов   |
+---------+
4 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id = 1;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
+----+---------+
1 row in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id = 3;
+----+---------+
| id | surname |
+----+---------+
|  3 | Сидоров |
+----+---------+
1 row in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id > 3;
+----+---------+
| id | surname |
+----+---------+
|  4 | Чижов   |
|  5 | Петров  |
|  6 | Петров  |
+----+---------+
3 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE id < 3;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
+----+---------+
2 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE surname = "Петров";
+----+---------+
| id | surname |
+----+---------+
|  2 | Петров  |
|  5 | Петров  |
|  6 | Петров  |
+----+---------+
3 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE surname = "Петров" LIMIT 2;
+----+---------+
| id | surname |
+----+---------+
|  2 | Петров  |
|  5 | Петров  |
+----+---------+
2 rows in set (0.00 sec)

mysql> SELECT * FROM teacher WHERE surname = "Петров" LIMIT 1;
+----+---------+
| id | surname |
+----+---------+
|  2 | Петров  |
+----+---------+
1 row in set (0.00 sec)

mysql> SELECT * FROM teacher LIMIT 5;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
|  3 | Сидоров |
|  4 | Чижов   |
|  5 | Петров  |
+----+---------+
5 rows in set (0.00 sec)

mysql> SELECT id AS 'ИДЕНТИФИКАТОР', surname AS 'Фамилия' FROM teacher;
+---------------+---------+
| ИДЕНТИФИКАТОР | Фамилия |
+---------------+---------+
|             1 | Иванов  |
|             2 | Петров  |
|             3 | Сидоров |
|             4 | Чижов   |
|             5 | Петров  |
|             6 | Петров  |
+---------------+---------+
6 rows in set (0.00 sec)

mysql> SELECT * FROM teacher ORDER BY surname;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
|  5 | Петров  |
|  6 | Петров  |
|  3 | Сидоров |
|  4 | Чижов   |
+----+---------+
6 rows in set (0.00 sec)

mysql> SELECT * FROM teacher ORDER BY id;
+----+---------+
| id | surname |
+----+---------+
|  1 | Иванов  |
|  2 | Петров  |
|  3 | Сидоров |
|  4 | Чижов   |
|  5 | Петров  |
|  6 | Петров  |
+----+---------+
6 rows in set (0.00 sec)

mysql> SELECT * FROM teacher ORDER BY id DESC;
+----+---------+
| id | surname |
+----+---------+
|  6 | Петров  |
|  5 | Петров  |
|  4 | Чижов   |
|  3 | Сидоров |
|  2 | Петров  |
|  1 | Иванов  |
+----+---------+
6 rows in set (0.00 sec)

mysql> ALTER TABLE teacher ADD age INT;
Query OK, 0 rows affected (0.02 sec)
Records: 0  Duplicates: 0  Warnings: 0

mysql> show columns from teacher;
+---------+--------------+------+-----+---------+----------------+
| Field   | Type         | Null | Key | Default | Extra          |
+---------+--------------+------+-----+---------+----------------+
| id      | int          | NO   | PRI | NULL    | auto_increment |
| surname | varchar(255) | NO   |     | NULL    |                |
| age     | int          | YES  |     | NULL    |                |
+---------+--------------+------+-----+---------+----------------+
3 rows in set (0.00 sec)

mysql> select * from teacher;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  | NULL |
|  2 | Петров  | NULL |
|  3 | Сидоров | NULL |
|  4 | Чижов   | NULL |
|  5 | Петров  | NULL |
|  6 | Петров  | NULL |
+----+---------+------+
6 rows in set (0.00 sec)

mysql> UPDATE teacher SET age = 20 where id=1;
Query OK, 1 row affected (0.00 sec)
Rows matched: 1  Changed: 1  Warnings: 0

mysql> select * from teacher;
+----+---------+------+
| id | surname | age  |
+----+---------+------+
|  1 | Иванов  |   20 |
|  2 | Петров  | NULL |
|  3 | Сидоров | NULL |
|  4 | Чижов   | NULL |
|  5 | Петров  | NULL |
|  6 | Петров  | NULL |
+----+---------+------+
6 rows in set (0.00 sec)

mysql> UPDATE teacher SET age = 25;
Query OK, 6 rows affected (0.00 sec)
Rows matched: 6  Changed: 6  Warnings: 0

ORDER BY 

# Синтаксис
SELECT список_полей
FROM таблица
WHERE условие_отбора
[ORDER BY поле1 [ASC | DESC ][, поле2 [ASC | DESC ]][, ...]]]

# Инструкция SELECT, содержащая предложение ORDER BY, 
# включает в себя следующие элементы:
# список_полей:
# Имена полей, извлекаемых вместе со всеми псевдонимами, 
# агрегатными функциями SQL, 
# предикатами отбора (ALL, DISTINCT, DISTINCTROW или TOP) 
# или с другими параметрами инструкции SELECT.

# таблица
# Имя таблицы, из которой извлекаются записи.

# условия_отбора
# Условия отбора. Если инструкция содержит предложение WHERE

# поле1, поле2
# Имена полей, по которым нужно выполнить сортировку записей.

Замечания
Предложение ORDER BY не является обязательным. Его следует использовать,
когда необходимо отобразить данные в отсортированном виде.

По умолчанию применяется порядок сортировки по возрастанию ASC (от А до Я, от 0 до 9).
В двух приведенных ниже примерах показана сортировка имен сотрудников по фамилиям.


SELECT LastName, FirstName
FROM Employees
ORDER BY LastName;

SELECT LastName, FirstName
FROM Employees
ORDER BY LastName ASC;


Чтобы выполнить сортировку по убыванию (от Я до А, от 9 до 0), 
добавьте зарезервированное слово DESC в конце каждого поля, 
по которому вы хотите отсортировать записи. 
В примере ниже выполняется сортировка фамилий 
сотрудников в порядке убывания заработной платы.


SELECT LastName, Salary
FROM Employees
ORDER BY Salary DESC, LastName;


Если в предложении ORDER BY указать поле, 
содержащее данные типа Memo или объекты OLE, возникнет ошибка. 
Ядро СУБД Microsoft Access не поддерживает сортировку по полям этих типов.

Предложение ORDER BY обычно является последним элементом в инструкции SQL.

В предложение ORDER BY можно включать дополнительные поля.
Сначала записи сортируются по полю, указанному в предложении ORDER BY первым. 
Затем для записей с одинаковыми значениями первого поля 
выполняется сортировка по полю, указанному вторым, и т. д.
###############################################################################

№3. Выберите название, описание и год выхода фильмов из таблицы film.
Отсортируйте полученный список по названию в алфавитном порядке
и выведите первые десять строк

select  title, description, release_year
from film
order by title ASC 
limit 10;

№4. Для удобства показа мы разобьем список фильмов 
на страницы по десять записей на каждой.
Для формирования третьей страницы списка выберите название, описание и год 
выхода фильмов из таблицы film.
Отсортируйте полученный список по названию в алфавитном порядке
и выведите десять строк начиная с двадцать первой.
_____________________________________

ИЗ ТЕХСОБЕСА Test_IT

SELECT COUNT(*) FROM employees - посчитать колличество строк(сотрудников и тп) в employees

SELECT COUNT(*) FROM employees WHERE status='active'

SELECT Emp_FirstName, Emp_MidleName, Emp_SecondName FROM Employees
JOIN Departament ON Dep_ID
WHERE Dep_ID='бухгалтерия'
________________________________

В SQL агрегатные функции используются для выполнения вычислений над набором значений и возвращают одно значение. Они полезны для анализа данных, например, для подсчета, суммирования, поиска среднего значения и т.д. Рассмотрим основные агрегатные функции, предикаты отбора и другие параметры инструкции `SELECT`.

### 1. Агрегатные функции SQL

**Основные агрегатные функции:**

- **`COUNT()`**: Подсчитывает количество строк в наборе. Можно использовать с `*` для подсчета всех строк или с конкретным столбцом для подсчета значений в этом столбце.

    ```sql
    SELECT COUNT(*) FROM employees;  -- общее количество строк
    SELECT COUNT(employee_id) FROM employees;  -- количество сотрудников с заданным ID
    ```

- **`SUM()`**: Суммирует значения в указанном столбце числового типа.

    ```sql
    SELECT SUM(salary) FROM employees;  -- общая сумма зарплат
    ```

- **`AVG()`**: Вычисляет среднее значение в указанном столбце.

    ```sql
    SELECT AVG(salary) FROM employees;  -- средняя зарплата сотрудников
    ```

- **`MIN()`**: Находит минимальное значение в указанном столбце.

    ```sql
    SELECT MIN(salary) FROM employees;  -- минимальная зарплата сотрудников
    ```

- **`MAX()`**: Находит максимальное значение в указанном столбце.

    ```sql
    SELECT MAX(salary) FROM employees;  -- максимальная зарплата сотрудников
    ```

### 2. Предикаты отбора

Предикаты отбора могут быть использованы в комбинации с агрегатными функциями и указанием условий в запросах:

- **`ALL`**: Указывает, что агрегатная функция должна учитывать все значения. Например, по умолчанию `COUNT`, `SUM`, `AVG` и другие функции уже используют `ALL`. Но его можно явно указать для ясности.

    ```sql
    SELECT SUM(salary) FROM employees WHERE department_id = 1;  -- учитываются все зарплаты в департаменте 1
    ```

- **`DISTINCT`**: Удаляет дублирующиеся значения при подсчете или суммировании.

    ```sql
    SELECT COUNT(DISTINCT department_id) FROM employees;  -- количество уникальных департаментов
    SELECT AVG(DISTINCT salary) FROM employees;  -- средняя уникальная зарплата
    ```

- **`DISTINCTROW`**: Используется в некоторых СУБД (например, MySQL). Он позволяет учитывать только уникальные строки в результаты, хотя в основном используют просто `DISTINCT`.

- **`TOP`**: Ограничивает количество строк, возвращаемых запросом. Используется, например, в SQL Server.

    ```sql
    SELECT TOP 5 * FROM employees ORDER BY salary DESC;  -- 5 самых высоких зарплат
    ```

### 3. Примеры использования

Можно комбинировать агрегатные функции с другими параметрами `SELECT`:

* Пример с использованием `COUNT`, `DISTINCT`, и `WHERE`:

    ```sql
    SELECT COUNT(DISTINCT job_title) 
    FROM employees 
    WHERE department_id = 1;  -- количество уникальных должностей в департаменте 1
    ```

* Пример с `SUM` и `HAVING` для фильтрации по агрегатным данным:

    ```sql
    SELECT department_id, SUM(salary) AS total_salary
    FROM employees
    GROUP BY department_id
    HAVING SUM(salary) > 100000;  -- департаменты с общей зарплатой больше 100000
    ```

### Заключение

Агрегатные функции SQL позволяют эффективно анализировать и обрабатывать данные. Использование предикатов отбора, таких как `ALL`, `DISTINCT`, и `TOP`, позволяет точнее настраивать запросы и получать необходимые результаты. Важно знать, как комбинировать эти функции и параметры для получения точной информации из базы данных.

_______________________________________

Чтобы посчитать количество строк в таблице с помощью SQL, вы можете использовать функцию агрегирования `COUNT()`. Вот как это можно сделать:

### Общее количество строк в таблице:

Если вы хотите получить общее количество строк в таблице, используйте следующий запрос:

```sql
SELECT COUNT(*) FROM table_name;
```

Здесь `table_name` замените на имя вашей таблицы. Это вернет одно число — общее количество строк в таблице.

### Подсчет строк с условиями:

Если вам нужно подсчитать строки с определенными условиями, добавьте оператор `WHERE`. Например:

```sql
SELECT COUNT(*) FROM table_name WHERE condition;
```

Замените `condition` на нужное условие. Например, если у вас есть поле `status`, и вам нужно посчитать количество строк, где `status` равно `active`, вы можете написать:

```sql
SELECT COUNT(*) FROM table_name WHERE status = 'active';
```

### Подсчет уникальных значений:

Если вы хотите подсчитать количество уникальных значений в определенном столбце, используйте следующий запрос:

```sql
SELECT COUNT(DISTINCT column_name) FROM table_name;
```

Здесь `column_name` — это имя столбца, в котором вы хотите получить количество уникальных значений. Например:

```sql
SELECT COUNT(DISTINCT user_id) FROM table_name;
```

Этот запрос вернет количество уникальных пользователей в столбце `user_id`.

### Пример:

Предположим, у вас есть таблица `employees`, и вы хотите узнать общее количество сотрудников. Ваш запрос будет выглядеть так:

```sql
SELECT COUNT(*) FROM employees;
```

Если вас интересуют только активные сотрудники, то запрос будет следующим:

```sql
SELECT COUNT(*) FROM employees WHERE status = 'active';
```

Таким образом, с помощью функции `COUNT()` можно легко подсчитать количество строк в таблице в зависимости от ваших требований.
_______________________________________
С помощью какого из следующих запросов
можно выбрать все записи из тбалицы Persons, 
у которых значение поля LastName алфавитно расположено между (и включая) Hansen и Pettersen?

SELECT *  
FROM Persons  
WHERE LastName BETWEEN 'Hansen' AND 'Pettersen';  

________________________________________

**HAVING** — это оператор в SQL, используемый для фильтрации результатов, которые возвращает оператор `GROUP BY`. Он позволяет ограничивать результаты группировок по определённым условиям.

### Особенности использования HAVING:
- Применяется после оператора `GROUP BY`.
- Чаще всего используется для работы с агрегатными функциями, такими как `SUM()`, `COUNT()`, `AVG()` и т.д.
- Позволяет фильтровать группы результатов, которые не удовлетворяют заданным условиям.

### Пример:
Предположим, у вас есть таблица `sales` с колонками `product_id`, `quantity` и `price`. Чтобы получить продукты, у которых общая сумма продаж превысила 100, вы можете использовать:

```sql
SELECT product_id, SUM(quantity) AS total_quantity
FROM sales
GROUP BY product_id
HAVING SUM(quantity) > 100;
```

В этом запросе:
- `GROUP BY product_id` группирует продажи по `product_id`.
- `HAVING` после группировки фильтрует только те группы, где сумма `quantity` превышает 100.


