...
Ниже структурированная таблица с попарным подходом тестирования для GET-эндпойнта последнего заказа в личном кабинете. Таблица делит сценарии на ввод, ожидаемый результат и ключевые проверки.

| № | Сценарий (описание) | Входные параметры | Ожидаемый статус/ответ | Ключевые проверки | Примечания |
|---|----------------------|--------------------|-------------------------|--------------------|------------|
| 1 | Позитивный: существующий клиент с заказом | customer_id: cust_001 | 200 OK | response.customer_id == cust_001; response.order_id не пуст; order принадлежит cust_001; сумма элементов корректна; response.price == Σ(price_i * qty_i); response.total_amount == Σ(price_i * qty_i) (без налогов/скидок) | Пример: items = [{price:10, qty:2},{price:5, qty:1}] -> price=25, total_amount=25 |
| 2 | Позитивный: клиент с одним элементом в заказе | customer_id: cust_002 | 200 OK | response.customer_id == cust_002; в заказе один элемент; корректность price и total_amount как у одного элемента | Пример: items = [{price:7, qty:1}] -> price=7, total_amount=7 |
| 3 | Негатив: несуществующий клиент | customer_id: nonexistent | 404 Not Found или 200 с пустым last_order согласно контракту | response.last_order отсутствует/равен null; сообщение об отсутствии заказа не должно ломать контракт | Зависит от реализации API; зафиксируйте контракт заранее |
| 4 | Пограничный: клиент без заказов | customer_id: cust_no_orders | 200 OK | response.last_order == null | Вариант: пустой заказ или специальное поле; проверьте контракт |
| 5 | Негатив: несоответствие суммы элементов и общей суммы | customer_id: cust_003 (с установленной тестовой фикстурой) | 200 OK | computed_price = Σ(price_i * qty_i); assert computed_price == response.price; assert computed_price == response.total_amount (при отсутствии налогов/скидок); иначе учесть налог/скидку | Выделить тест на консистентность данных |
| 6 | Попарное: валидность клиента + наличие заказа | customer_valid: {existing_with_order, existing_no_order, nonexistent} | 200/404 в зависимости | сочетание customer_id и наличие order; проверка соответствия | Комбинации создаются через попарное покрытие |
| 7 | Попарное: количество элементов в заказе | item_count: {1, 3} | 200 OK или 404/200 с last_order null (если нет заказа) | при существовании заказа проверить, что число элементов совпадает; сумма внутри заказа корректна | Учитывайте разные конфигурации price/qty внутри заказа |
| 8 | Попарное: наличие налогов/скидок | has_tax_or_discount: {yes, no} | 200 OK | если есть налог/скидка, total_amount должны учитывать это; price должен соответствовать сумме элементов | Налоги/скидки должны применяться согласно бизнес-правилам |
| 9 | Валидация схемы Swagger/OpenAPI | любые валидные данные | 200 OK; структура соответствует схеме | валидировать: поля customer_id, order_id, items, price, total_amount; типы и форматы | Можно использовать jsonschema/OpenAPI валидаторы |

Как это использовать на практике
- Введите конкретные фикстуры БД, чтобы данные соответствовали каждому сценарию (cust_001, cust_002, cust_no_orders, nonexistent и т. д.).
- Реализуйте тестовые кейсы в выбранном фреймворке (pytest, JUnit, NUnit и т. д.), применяя pairwise-генерацию для сценариев 6–8.
- Добавьте asserts:
  - Проверку соответствия идентификаторов: response.customer_id == запрошенный customer_id
  - Проверку валидности order_id для данного customer_id
  - Вычисление суммы элементов и сравнение с response.price и response.total_amount
  - Обработку случаев без заказов (last_order == null) и несуществующего клиента
